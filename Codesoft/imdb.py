...
import numpy as np  # Import numpy for root mean squared error calculation
...
predicted_rating = model.named_steps['linear_regression'].predict(sample_vectorized)
...
plt.subplot(1, 3, 1)
plt.scatter(y_test, test_predictions, color='skyblue', alpha=0.6)
...
plt.subplot(1, 3, 2)
plt.bar(['Actual', 'Predicted'], [y_test.iloc[0], predicted_rating[0]], color=['cornflowerblue', 'lightgreen'], alpha=0.8)
